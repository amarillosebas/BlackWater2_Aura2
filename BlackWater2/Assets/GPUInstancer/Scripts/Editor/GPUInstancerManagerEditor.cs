        ...
        public void DrawDebugBox(GPUInstancerEditorSimulator simulator = null)
        {
            if (!Application.isPlaying)
            {
                ...
                if (showDebugBox)
                {
                    ...
                    {
                        EditorGUILayout.HelpBox("Simulation is kept alive. The simulation will not show newly painted terrain details. Changing some prototype settings during the simulation can also cause errors. Please stop and start the simulation again to solve these issues.", MessageType.Warning);
                    }
                }
